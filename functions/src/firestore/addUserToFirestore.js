...
exports.addUserToFirestore = functions.auth.user().onCreate(async (user) => {
  if (!user.uid) {
    printDevLogs(
      S_AUTF_E_PATH,
      S_AUTF_E_INFO,
      `ERROR: Failed to add user to firestore ${FS_USER_COLLECTION_KEY} collection --> the uid in the user object is null &&&& DATA: uid: ${user.uid}`
    );
    return false;
  }
  ...
  try {
    ...
    printDevLogs(
      S_AUTF_E_PATH,
      S_AUTF_E_INFO,
      `ERROR: Failed to add user to firestore ${FS_USER_COLLECTION_KEY} collection --> ${e} &&&& DATA: uid: ${user.uid}, email: ${user.email}, username: ${user.displayName}`
    );
    return false;
  }
});
